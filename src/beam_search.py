...
def candidates_from_logits(tokenizer, mask_logits, n_beams=5, chosen_pokemon=None, prob=0, forbidden_pokemon=None):
    ...
    k = min(k, mask_logits.shape[0])
    top_k = torch.topk(mask_logits, k)
    # print(top_n)
    # limit top k to be at most the number of tokens
    range_lim = min(n_beams + len(forbidden_pokemon), mask_logits.shape[0])
...
def beam_step_from_batch(logits, tokenizer, chosen_pokemon, prob, n_beams=5, forbidden_pokemon=None):
    if forbidden_pokemon is None:
        forbidden_pokemon = []
    mask_logits = torch.softmax(logits, dim=0)
    return candidates_from_logits(tokenizer, mask_logits, n_beams=n_beams, chosen_pokemon=chosen_pokemon, prob=prob,
